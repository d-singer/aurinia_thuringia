...
writexl::write_xlsx(summary %>% filter(weightcum <= 0.95) %>% 
                      select(!weightcum),"data/population_models/models_summary_kriegberg_female_weight95.xlsx")
...
hist <- capt.hist.gof[3:11] %>% as.matrix()
...
df <- data.frame(date = sort(unique(falter_complete$date)),
                 ...
                 Phi.ucl = c( mymod.Phi$ucl, NA)) %>% merge(weather %>% filter(date %in% falter_complete$date), by="date")

data.table::fwrite(df, "data/kriegberg_daily_estimates_females.csv")
